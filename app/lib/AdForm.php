...
class AdForm extends Form
{
    ...
    {
        if ($this->validate()) {
            $img = !empty($old_data['img'])?$old_data['img']:false;
            $img_other = !empty($old_data['img_other'])?$old_data['img_other']:false;
            $img_other2 = !empty($old_data['img_other2'])?$old_data['img_other2']:false;
            $title = $this->model->getDb()->escape($this->title);
            $category = $this->model->getDb()->escape($this->category);
            $id_user = Session::get('id');
            $text = $this->model->getDb()->escape($this->description);
            $price = $this->model->getDb()->escape($this->price);
...
//            $file = $this->model->getDb()->escape($this->email);
            $region = $this->model->getDb()->escape($this->region);
